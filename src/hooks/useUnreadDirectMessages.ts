  ...
  useEffect(() => {
    ...
    try {
      ...
      const unsubscribeChats = onSnapshot(chatsQuery, (snapshot) => {
        ...
        console.warn('Direct messages real-time snapshot warning:', err);
        setLoadingChats(false);
      });

      return () => unsubscribeChats();
    } catch (err) {
      console.warn('Error setting up DM listener:', err);
      setLoadingChats(false);
    }
  }, [currentUid]);
  ...
  useEffect(() => {
    ...
    try {
      ...
      const unsubscribeNotifs = onSnapshot(notifsQuery, (snapshot) => {
        ...
        console.warn('DM notifications real-time snapshot warning:', err);
        setLoadingNotifs(false);
      });

      return () => unsubscribeNotifs();
    } catch (err) {
      console.warn('Error setting up DM notifications listener:', err);
      setLoadingNotifs(false);
    }
  }, [currentUid]);
