      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {mode === 'signup' && (
            <div>
              <label className="text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase font-mono tracking-wider mb-1 block">
                Account Role (6 Ecosystem Roles)
              </label>
              <select
                value={role}
                onChange={(e) => setRole(e.target.value as UserRole)}
                className="w-full px-3 py-2.5 rounded-xl bg-slate-50 dark:bg-black/60 border border-slate-200 dark:border-white/15 text-xs text-[#263238] dark:text-white focus:outline-none focus:border-[#FF6A00] font-bold uppercase cursor-pointer"
              >
                <option value="member">Member (Community & Sports Network)</option>
                <option value="athlete">Athlete (Player Profile, Highlights & Stats)</option>
                <option value="coach">Coach (Team Roster & Game Schedule)</option>
                <option value="organization">Organization (Tournament Host & Programs)</option>
                <option value="scout">Scout / Recruiter (Verified Player Matrix)</option>
                <option value="creator">Content Creator (Media & Photography Studio)</option>
                <option value="viewer">Parent / Fan / Spectator (Live Feed & Highlights)</option>
              </select>
            </div>
          )}
