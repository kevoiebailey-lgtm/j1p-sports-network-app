...
export const CyberRoleLayout: React.FC<CyberRoleLayoutProps> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    // Listen to events collection for live delays / active announcements
    const unsubTourn = onSnapshot(
      query(collection(db, 'events'), limit(5)),
      (snap) => {
        if (!snap.empty) {
      ...
      (err: any) => {
        if (err?.code !== 'permission-denied') {
          console.warn('Featured events announcement snapshot notice:', err?.message || err);
        }
      }
    );
    ...
    const unsubBroadcasts = onSnapshot(
      ...
      (err: any) => {
        if (err?.code !== 'permission-denied') {
          console.warn('Broadcasts banner snapshot notice:', err?.message || err);
        }
      }
    );
