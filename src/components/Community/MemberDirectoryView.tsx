...
import { INITIAL_ATHLETES } from '../../lib/mockData';
...
export const getMemberConnections = (member: any, liveFollowCount: number = 0): number => {
  if (!member) return 45;
  const uid = String(member.uid || member.id || '');
  let seed = 0;
  for (let i = 0; i < uid.length; i++) {
    seed = (seed * 31 + uid.charCodeAt(i)) % 1000;
  }

  let baseConnections = 45;
  const role = String(member.role || 'athlete').toLowerCase();
  ...
  if (Boolean(member.isVerified)) baseConnections += 160;
  if (Array.isArray(member.mediaUrls) && member.mediaUrls.length > 0) baseConnections += member.mediaUrls.length * 20;

  return baseConnections + (typeof liveFollowCount === 'number' ? liveFollowCount : 0);
};
...
export const getMemberInitials = (name?: string): string => {
  if (!name || typeof name !== 'string' || !name.trim()) return 'J1';
  const parts = name.trim().split(/\s+/).filter(Boolean);
  if (parts.length === 0) return 'J1';
...
export const MemberDirectoryView: React.FC = () => {
  ...
  // Real-time Fetch: Users Collection (Direct collection query, Null-Safe Document Mapping)
  useEffect(() => {
    let isMounted = true;
    setLoading(true);
    ...
    const mapUserDoc = (docSnap: any): UserProfile => {
      ...
      const safeRole = String(data.role || 'athlete').toLowerCase();
      const safeSport = String(data.sport || 'Multi-Sport');
      const safeGradYear = String(data.gradYear || data.classOf || '');
      const safeState = String(data.state || '').trim();
      const rawName = data.displayName || data.name || (data.email ? data.email.split('@')[0] : 'Member');
      const safeDisplayName = String(rawName || 'Member').trim() || 'Member';
      const safeAvatar = String(data.avatarUrl || data.photoURL || '');
      ...
      return {
        ...
        sport: safeSport as SportType,
        avatarUrl: safeAvatar,
        photoURL: safeAvatar,
        bio: String(data.bio || ''),
        isVerified: Boolean(data.isVerified),
        status: String(data.status || 'active'),
        highSchool: String(data.highSchool || data.school || data.schoolName || ''),
        state: safeState,
        position: String(data.position || data.primaryPosition || ''),
        gradYear: safeGradYear,
        jerseyNumber: String(data.jerseyNumber || ''),
        teamName: String(data.teamName || data.clubTeam || ''),
    ...
    try {
      // Direct query on users collection without compound where clauses
      const usersRef = collection(db, 'users');
      const unsubscribe = onSnapshot(usersRef, (snapshot) => {
        if (!isMounted) return;
        if (!snapshot.empty) {
          const fetchedList = snapshot.docs.map(mapUserDoc);
        ...
        if (!isMounted) return;
        // On snapshot error, attempt one-shot getDocs fallback
        getDocs(usersRef)
          .then((snap) => {
            if (!isMounted) return;
            if (!snap.empty) {
  ...
  // Filtered & Sorted Members (In-Memory Pure Client-Side Filtering)
  const filteredMembers = useMemo(() => {
    return users.filter(member => {
      ...
      if (searchQuery.trim()) {
        ...
        const matchesName = String(member.displayName || '').toLowerCase().includes(queryClean);
        const matchesEmail = String(member.email || '').toLowerCase().includes(queryClean);
        const matchesSchool = String(member.highSchool || '').toLowerCase().includes(queryClean);
        const matchesTeam = String(member.teamName || '').toLowerCase().includes(queryClean);
        const matchesPosition = String(member.position || '').toLowerCase().includes(queryClean);
        const matchesState = String(member.state || '').toLowerCase().includes(queryClean);
        const matchesSport = String(member.sport || '').toLowerCase().includes(queryClean);
        const matchesGradYear = String(member.gradYear || '').includes(queryClean);

        if (!matchesName && !matchesEmail && !matchesSchool && !matchesTeam && !matchesPosition && !matchesState && !matchesSport && !matchesGradYear) {
          return false;
        }
      }
      ...
      // 2. Role Filter Tabs
      if (selectedRole !== 'all') {
        const memberRole = String(member.role || 'athlete').toLowerCase();
        if (selectedRole === 'athlete' && memberRole !== 'athlete') return false;
        if (selectedRole === 'coach' && memberRole !== 'coach') return false;
        if (selectedRole === 'scout' && memberRole !== 'scout' && memberRole !== 'recruiter') return false;
        if (selectedRole === 'director' && memberRole !== 'director') return false;
        if (selectedRole === 'viewer' && memberRole !== 'viewer' && memberRole !== 'fan') return false;
      }

      // 3. Sport Dropdown Filter
      if (selectedSport !== 'All Sports') {
        const mSport = String(member.sport || '').toLowerCase();
        const selSport = selectedSport.toLowerCase();
        if (mSport !== selSport && !mSport.includes(selSport) && !selSport.includes(mSport)) {
          return false;
        }
      }
      ...
      if (selectedGradYear !== 'all') {
        const mYear = String(member.gradYear || '').trim();
        if (selectedGradYear === '2029') {
          const numYear = parseInt(mYear, 10);
          if (isNaN(numYear) || numYear < 2029) return false;
        } else if (mYear !== selectedGradYear) {
          return false;
        }
      }
      ...
      if (selectedState !== 'all') {
        if (String(member.state || '').trim().toUpperCase() !== selectedState.toUpperCase()) return false;
      }

      // 6. Quick Toggles
      if (verifiedOnly && !member.isVerified) return false;
      if (followingOnly && !effectiveFollowingUids.includes(memberUid)) return false;
      if (hasFilmOnly && (!Array.isArray(member.mediaUrls) || member.mediaUrls.length === 0)) return false;
      ...
      if (filterByAthleteFollowed !== 'all') {
        ...
        const athletesFollowed = getAthletesFollowedBy(memberUid);
      ...
      if (filterByRecruiterFollower !== 'all') {
        ...
        const recruitersFollowing = getRecruitersFollowing(memberUid);
      ...
      if (sortBy === 'gradYear') {
        return String(a.gradYear || '9999').localeCompare(String(b.gradYear || '9999'));
      }
      if (sortBy === 'followers' || sortBy === 'connections') {
        const countA = getMemberConnections(a, followerCounts[a.uid || (a as any).id] || 0);
        const countB = getMemberConnections(b, followerCounts[b.uid || (b as any).id] || 0);
  ...
  const followedAthletesOptions = useMemo(() => {
    ...
    return users.filter(u => athleteIds.has(u.uid || (u as any).id));
  ...
  const activeRecruitersOptions = useMemo(() => {
    ...
    return users.filter(u => recruiterIds.has(u.uid || (u as any).id));
  }, [users, allConnections]);
  ...
  // Dynamic Aggregate Stats calculated directly from loaded user documents
  const stats = useMemo(() => {
    const total = users.length;
    const athletes = users.filter(u => String(u.role || '').toLowerCase() === 'athlete').length;
    const coachesAndScouts = users.filter(u => {
      const r = String(u.role || '').toLowerCase();
      return r === 'coach' || r === 'scout' || r === 'recruiter';
    }).length;
    const verified = users.filter(u => Boolean(u.isVerified)).length;
    const totalConnections = users.reduce((acc, u) => acc + getMemberConnections(u, followerCounts[u.uid || (u as any).id] || 0), 0);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4">
        ...
        {activeTabMode === 'directory' && (
          <div className="flex items-center gap-1.5 overflow-x-auto pb-2 scrollbar-none mb-3">
            {ROLE_TABS.map((tab) => {
              const count = tab.role === 'all'
                ? users.length
                : tab.role === 'scout'
                ? users.filter(u => { const r = String(u.role || '').toLowerCase(); return r === 'scout' || r === 'recruiter'; }).length
                : tab.role === 'viewer'
                ? users.filter(u => { const r = String(u.role || '').toLowerCase(); return r === 'viewer' || r === 'fan'; }).length
                : users.filter(u => String(u.role || '').toLowerCase() === tab.role).length;

              return (
                <button
                  key={tab.role}
                  onClick={() => setSelectedRole(tab.role)}
                  className={`px-3.5 py-1.5 rounded-xl text-xs font-bold font-mono uppercase tracking-wider transition-all cursor-pointer whitespace-nowrap flex items-center gap-1.5 ${
                ...
                >
                  ...
                  <span className={`px-1.5 py-0.5 rounded-full text-[10px] font-mono ${
                    selectedRole === tab.role ? 'bg-black/20 text-slate-950 font-black' : 'bg-white/10 text-slate-300'
                  }`}>
                    {count}
                  </span>
                </button>
            ...
            {loading ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 animate-pulse">
                {[...Array(8)].map((_, i) => (
                  <div
                    key={i}
                    className="bg-[#131C2E] border border-[#24324F] rounded-2xl p-4 flex flex-col justify-between min-h-[340px]"
                  >
                    <div>
                      <div className="flex items-center justify-between mb-3.5">
                        <div className="w-20 h-4 bg-slate-800 rounded-lg"></div>
                      ...
                      <div className="w-full h-8 bg-[#090D16] border border-[#24324F] rounded-xl mb-3"></div>
                      <div className="grid grid-cols-3 gap-1.5 bg-[#090D16]/80 p-2 rounded-xl border border-[#24324F] mb-3">
                        <div className="h-6 bg-slate-800 rounded"></div>
                        <div className="h-6 bg-slate-800 rounded"></div>
                        <div className="h-6 bg-slate-800 rounded"></div>
                      </div>
                    </div>
                    <div className="flex items-center gap-2 pt-3 border-t border-[#24324F]">
                      <div className="flex-1 h-9 bg-slate-800 rounded-xl"></div>
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-3 gap-1.5 bg-[#090D16]/80 p-2 rounded-xl border border-[#24324F] mb-3 text-center">
                          <div>
                            ...
                            <span className="text-[11px] font-bold font-mono text-[#FFC857]">{member.gradYear ? `'${String(member.gradYear).slice(-2)}` : '--'}</span>
                          </div>
                        ...
                        return (
                          <tr key={member.uid} className="hover:bg-[#1A253D]/50 transition-colors">
                            ...
                            <td className="py-3 px-4 font-mono font-bold text-[#FFC857]">
                              {member.gradYear ? `'${String(member.gradYear).slice(-2)}` : '--'}
                            </td>
