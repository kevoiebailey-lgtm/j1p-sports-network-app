...
export const AdminOverviewPage: React.FC<AdminOverviewPageProps> = ({ 
    ...
    try {
      ...
      const unsubscribe = onSnapshot(usersRef, (snapshot) => {
        ...
        if (err?.code !== 'permission-denied') {
          console.warn('Live Firestore users subscription notice:', err?.message || err);
        }
        setLoading(false);
      });
      ...
      if (err?.code !== 'permission-denied') {
        console.warn('Error attaching users listener:', err?.message || err);
      }
      setLoading(false);
    }
  }, [isUserAdmin]);
  ...
  useEffect(() => {
    ...
    try {
      ...
      const unsubscribe = onSnapshot(eventsRef, (snapshot) => {
        ...
        if (err?.code !== 'permission-denied') {
          console.warn('Live Firestore events subscription notice:', err?.message || err);
        }
      });
      ...
      if (err?.code !== 'permission-denied') {
        console.warn('Error attaching events listener:', err?.message || err);
      }
    }
  }, [isUserAdmin]);
  ...
  useEffect(() => {
    ...
    // Only subscribe to orders if authenticated as admin/director
    const isUserAdmin = isAdmin || user.email === 'kevoiebailey@gmail.com' || profile?.role === 'admin' || profile?.role === 'director';
    ...
    try {
      ...
      const unsubOrders = onSnapshot(ordersRef, (snapshot) => {
        ...
        console.info('Orders live listener notice:', err.message || err);
      });
      ...
      console.info('Error attaching revenue listener:', err);
    }
