...
class NotificationService {
  ...
  public async initFCM(userUid?: string): Promise<string | null> {
    ...
    // Only attempt FCM token registration if the user has actively granted notification permission
    ...
    try {
      ...
      const token = await getToken(messaging, {
        ...
        // Handled silently with Firestore real-time fallback
        console.info('[FCM] Background push not supported or restricted in environment; using Firestore real-time fallbacks.');
        return null;
      });
      ...
      console.info('[FCM] Setup notice (using Firestore real-time messaging fallback).');
    }
    return null;
  }
