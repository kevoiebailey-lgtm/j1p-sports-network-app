...
export function resolveFirebaseConfig(): FirebaseOptions & { firestoreDatabaseId?: string } {
  ...
  // Authoritative config from firebase-applet-config.json takes precedence to protect against legacy env values
  const apiKey = 
  ...
  const authDomain = rawAuthDomain === 'just1play.firebaseapp.com' ? 'just1play26.firebaseapp.com' : rawAuthDomain;
  ...
  const rawProjectId = 
    ...
    'just1play26';
  const projectId = rawProjectId === 'just1play' ? 'just1play26' : rawProjectId;
  ...
  const storageBucket = rawStorageBucket === 'just1play.firebasestorage.app' ? 'just1play26.firebasestorage.app' : rawStorageBucket;
  ...
  const appId = rawAppId === '1:256996216773:web:6a999608f77f0f254385ac' ? '1:256996216773:web:33ff47765011e8414385ac' : rawAppId;
...
// Standard GoogleAuthProvider without any continueUrl, custom redirect URI, or ActionCodeSettings
export const googleProvider = new GoogleAuthProvider();

// Initialize Firebase Storage explicitly targeting custom bucket URL
 ...
 * Strictly uses the standard (default) database without custom database parameter.
 */
export function getFirestoreDb(): Firestore {
  ...
  if (typeof window === 'undefined') {
    try {
      ...
      return initializeFirestore(app, memorySettings);
    } catch {
  ...
  try {
    ...
    console.log(`⚡ [Firestore] Multi-Tab Persistence Enabled (CACHE_SIZE_UNLIMITED) for standard (default) database`);
    return instance;
  } catch (err: any) {
    ...
    try {
      ...
      const fallbackInstance = initializeFirestore(app, memorySettings);
      console.log(`🛡️ [Firestore] Resilient memoryLocalCache active for standard (default) database`);
