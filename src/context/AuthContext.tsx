...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Google Sign In (Handles both Mobile/PWA Redirect and Desktop Popup with automatic fallback)
  const signInWithGoogle = async (selectedRole: UserRole = 'athlete'): Promise<UserProfile | null> => {
    setLoading(true);
    try {
      await ensureAuthReady();
      const isPwaMobile = isStandaloneOrMobileDevice();

      if (isPwaMobile) {
        console.log('[PWA Auth] Standalone PWA / Mobile device detected -> Using signInWithRedirect');
        try {
          sessionStorage.setItem('just1play_pending_auth_role', selectedRole);
          localStorage.setItem('just1play_pending_auth_role', selectedRole);
        } catch (e) {
          console.warn('[PWA Auth] Unable to save pending role in storage:', e);
        }
        await signInWithRedirect(auth, googleProvider);
        return null; // Page will redirect to Google account selector
      }

      // Desktop non-standalone environment: attempt signInWithPopup
      try {
        const result = await signInWithPopup(auth, googleProvider);
        return await handleGoogleUserResolved(result.user, selectedRole);
      } catch (popupErr: any) {
        console.warn('Google Sign-In popup attempt notice:', popupErr?.code, popupErr?.message);
        
        // If popup was blocked by browser or cancelled, gracefully fallback to redirect
        if (popupErr?.code === 'auth/popup-blocked' || popupErr?.code === 'auth/cancelled-popup-request') {
          console.warn('[PWA Auth] Popup blocked/cancelled. Falling back to signInWithRedirect for user compatibility.');
          try {
            sessionStorage.setItem('just1play_pending_auth_role', selectedRole);
            localStorage.setItem('just1play_pending_auth_role', selectedRole);
          } catch {}
