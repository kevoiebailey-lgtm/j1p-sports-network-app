...
async function startServer() {
  ...
  // In development mode (AI Studio sandbox), dev server must bind to port 3000.
  // In production (Cloud Run), the container must listen on process.env.PORT (defaults to 8080 in Cloud Run).
  ...
  app.post("/api/creator/upload-asset", async (req, res) => {
    try {
      ...
      // 1. Always save to local persistent disk first for guaranteed availability
      const localDiskPath = path.join(process.cwd(), "uploads", storagePath);
      await fs.promises.mkdir(path.dirname(localDiskPath), { recursive: true });
      await fs.promises.writeFile(localDiskPath, fileBuffer);

      let downloadUrl = `/uploads/${storagePath}`;

      // 2. Mirror to Firebase Admin Storage if service credentials have write permissions
      try {
        const bucket = getAdminStorage();
        ...
        console.log(`[Upload Asset Disk Pipeline] Stored asset ${storagePath} to local persistent disk (${localDiskPath}).`);
      }
  ...
  // API Health & Cloud Run Probe Endpoints
  app.get(["/api/health", "/healthz", "/_ah/health", "/livez", "/readyz"], (_req, res) => {
...
Return ONLY valid JSON matching this schema:
{
  ...
  // In production, if Cloud Run assigned a port other than 3000, also attempt to bind 3000 as secondary
  if (isProduction && PORT !== 3000) {
    try {
      ...
      secondaryServer.on("error", (err: any) => {
        console.log(`Port 3000 secondary listener notice: ${err?.message || err}`);
      });
    } catch (_) {}
  }
